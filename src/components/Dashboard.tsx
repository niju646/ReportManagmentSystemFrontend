...
import { Loader2, RefreshCw, LogOut, ArrowLeft, Eye, Download, Plus, CheckCircle } from "lucide-react";
import Papa from "papaparse";
import Select from "react-select";
import toast from "react-hot-toast";
import MessageViewer from "./MessageViewer"; // Import MessageViewer for direct rendering
...
const Dashboard: React.FC = () => {
  ...
  const [selectedRecipient, setSelectedRecipient] = useState<string | null>(null); // New state for recipient
  ...
  // Updated handleViewReport to show message details for a specific recipient
  const handleViewReport = async (notificationId: number, recipient: string, notificationType: string) => {
    setSelectedNotificationId(notificationId);
    setSelectedRecipient(recipient); // Set the recipient (phone for WhatsApp, email for email)
    setLoading(true);
    setError("");
    try {
      ...
      if (response.success) {
        ...
        console.log("Raw getReport response:", response.data);

        // Normalize the report data
        const normalizedReport = {
          ...response.data,
          type: response.data.type || "unknown",
          statuses: response.data.statuses.map((status: any) => {
            const isWhatsAppOrSMS = response.data.type === "whatsapp" || response.data.type === "sms";
            const recipientField = isWhatsAppOrSMS
              ? status.recipientPhone || status.recipient || "N/A" // Prioritize phone for WhatsApp/SMS
              : status.recipientEmail || status.recipient || "N/A"; // Use email for email notifications

            return {
              ...status,
              recipient: recipientField,
              recipientName: status.recipientName || "Unknown",
              status: status.status || "unknown",
              dateUpdated: status.dateUpdated || new Date().toISOString(),
              type: status.type || response.data.type,
            };
          }),
        };

        // Log normalized report for debugging
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {selectedNotificationId && selectedRecipient ? (
          <div>
            <button
              onClick={() => {
              ...
              className="mb-6 inline-flex items-center px-4 py-2 bg-blue border border-gray-300 rounded-md font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all"
            >
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
                <MessageViewer notificationId={selectedNotificationId} recipient={selectedRecipient} />
              </div>
            </div>
          </div>
        ) : (
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredNotifications.flatMap((notification) =>
                          notification.recipients.map((recipient: any) => (
                            <tr key={`${notification.notificationId}-${recipient.recipientEmail || recipient.recipientPhone}`} className="hover:bg-gray-50">
                                  ...
                                  onClick={() =>
                                    handleViewReport(
                                      ...
                                      notification.type === "whatsapp" || notification.type === "sms"
                                        ? recipient.recipientPhone || recipient.recipientEmail
